...
| `dedup-pr-runs` | no | `false` | De-duplicate the heavy path (`build` + `it-postgres`) across the `push` and `pull_request` events for same-repo PRs: skip the redundant `pull_request` copy (the branch's `push` run covers the same head SHA). Never skips the `push` run (keeps `branch-deploy` and the main deploy gates); fork PRs still run the heavy path. Verify branch-protection required checks on a live PR before enabling. See [.github/actions/ci-context/README.md](.github/actions/ci-context/README.md). |
